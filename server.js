...
// Also serve it from /admin (nicer URL)
app.get('/admin', (req, res) => {
     ...
     //   }
        ...
        const newUser = {
            ...
            kyc: {
                type: kycType,
                number: kycNumber,
                documentPath: '/uploads/' + kycFile.filename,
                submittedAt: new Date(),
                status: 'pending'
            },
            approvedAt: null
...
// CHECK KYC STATUS - ADD THIS HERE
...
// CHECK SESSION API - Add this new code
...
// GET USER BALANCE API - Add this new code
...
// GET USER STATUS (including frozen)
...
// LOGOUT API (for main website only)
app.post('/api/logout', (req, res) => {
    // Only clear user session, keep admin session
...
app.get('/api/admin/users', async (req, res) => {
    try {
        ...
        // Get all users (don't send passwords!)
...
app.post('/api/admin/update-balance', async (req, res) => {
    try {
        ...
        if (difference > 0) {
            ...
            console.log('ℹ️ No change in balance');
        }
...
async function saveTransaction(userEmail, type, amount, status, description) {
    try {
        ...
        await transactions.insertOne({
            userEmail: userEmail,
            type: type,           // 'deposit', 'withdraw', 'trade'
            amount: amount,
...
// Win Mode settings (store in memory)
let winModeEnabled = false;
let winModeProfit = 2;  // Default 2%
...
app.get('/api/admin/win-mode', (req, res) => {
    ...
    res.json({ success: true, winMode: winModeEnabled, profitPercent: winModeProfit });
});

// Set win mode
app.post('/api/admin/win-mode', (req, res) => {
    if (!req.session.admin) {
        return res.json({ success: false, message: 'Not authorized' });
    }
    const { winMode, profitPercent } = req.body;
    winModeEnabled = winMode;
    if (profitPercent !== undefined) {
        winModeProfit = profitPercent;
    }
    console.log(`Win mode: ${winModeEnabled ? 'ON' : 'OFF'} ${winModeEnabled ? `(Profit: ${winModeProfit}%)` : ''}`);
...
// ADMIN: APPROVE/REJECT USER (approves both account and KYC)
app.post('/api/admin/approve-user', async (req, res) => {
    try {
        ...
        // Update both account status AND KYC status
        const result = await users.updateOne(
            { email: email },
            { $set: { 
                status: status,
                'kyc.status': status,  // Also update KYC status
                approvedAt: new Date()
            } }
        );
